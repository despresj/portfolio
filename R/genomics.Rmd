...
load("~/Documents/MSU/STT-802_Statistical_Computation/homework/Xy.RData")
```
...
**2** Produce the following plot (using un-adjusted p-values) - Manhattan plot (-log10(pvalue) in the y-axis versus position of the SNP (1:ncol(X)) in the horizontal axis)

```{r}
plot(1:ncol(X), -log10(pvals))
```
...
```{r}
x_tiles <- quantile(pvals, prob = seq(from = 0, to = 1, by = 1 / length(pvals)))
theoretical <- seq(from = 0, to = 1, by = 1 / length(pvals))

plot(x = -log10(x_tiles), y = -log10(theoretical))
abline(a = 0, b = 1, col = "red")
abline(a = 2.5, b = 0, lty = 2, col = 4)
```

**At around 2.5 you start seeing the skew**

Hints: - To get the empirical quantiles of your p-values you can use `quantile(pvalues,prob=seq(from=0,to=1,by=1/length(pvalues))` 
